...
class SpotifyPlayer {
  constructor(options = {}) {
    ...
    this.exchangeHost = options.exchangeHost || 'https://dice-spotify-player.herokuapp.com';
      ...
      const loop = () => {
        if (!this.obtainingToken) {
          this.fetchPlayer()
            ...
            this.fetchFeatures()
              .then(data2 => {
                if (data2 !== null && data2.item !== null) {
                  this.dispatch('update2', data2);
                }
              })
            this.fetchArtist()
              .then(data3 => {
                if (data3 !== null && data3.item !== null) {
                  this.dispatch('update3', data3);
                }
              })
            .catch(e => {
  ...
  fetchFeatures() {
    return this.fetchGeneric('https://api.spotify.com/v1/audio-features/' + trackID).then(response => {
      if (response.status === 401) {
        return this.fetchToken()
          .then(tokenResponse => {
            if (tokenResponse.status === 200) {
  ...
  fetchArtist() {
    return this.fetchGeneric('	https://api.spotify.com/v1/artists/' + artistID).then(response => {
      if (response.status === 401) {
        return this.fetchToken()
          .then(tokenResponse => {
            if (tokenResponse.status === 200) {
